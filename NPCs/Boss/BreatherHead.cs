...
{
    [AutoloadBossHead]
    public class BreatherHead : ModNPC
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Exhalius, the Guardian");
        }
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            if (npc.life <= 0)
            {
                Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/BreatherHead_G"), 1f);
            }
        }

        public override void NPCLoot()
        {
            if (NPC.AnyNPCs(mod.NPCType("CorruptionBoss")))
            {
                Main.PlaySound(41, (int)npc.position.X, (int)npc.position.Y, 17);
                Main.NewText("[c/902ee1:Amalgamus has been detached, now active and vulnerable!]");
            }
        }
        ...
        public override bool PreAI()
        {
            if (Main.netMode != 1)
            {
                if (npc.ai[0] == 0)
                {
                    ...
                    {
                        latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("BreatherBody"), npc.whoAmI, 0, latestNPC);
                        Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
                    }
                    latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("BreatherTail"), npc.whoAmI, 0, latestNPC);
                    Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
            ...
            if ((double)npc.life < (double)npc.lifeMax * 0.35)
            {
                ...
                Vector2 vector8 = new Vector2(npc.position.X + (npc.width * 0.5f), npc.position.Y + (npc.height * 0.5f));
                {
                    ...
                    npc.velocity.Y = (float)(Math.Sin(rotation) * 12.5) * -1;
                }
            }


            npc.ai[2] += 0;
            if ((double)npc.life < (double)npc.lifeMax * 0.08)
            {
                if (Main.expertMode)
